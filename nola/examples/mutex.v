...
Section mutex_bor.
  ...
  Lemma mutex_bor_try_acquire {α l Px q} :
    [[{ mutex_bor α l Px ∗ q.[α] }]][inv_wsat ∗ borrow_wsat bupd ⟦⟧ᶜ]
      try_acquire_mutex [ #l]
  ...
  Lemma mutex_bor_lend_new {α l Px b q} :
    l ↦ #b -∗ ⟦ Px ⟧ᶜ -∗ q.[α] =[inv_wsat ∗ borrow_wsat bupd ⟦⟧ᶜ]=∗◇
  ...
  Lemma twp_iter_mblist {α Φx c l q} {f : val} {k n : nat} :
    (∀ l', [[{ ⟦ Φx (l' +ₗ 1) ⟧ᶜ }]][inv_wsat ∗ borrow_wsat bupd ⟦⟧ᶜ]
        f [ #(l' +ₗ 1)] [[{ RET #☠; ⟦ Φx (l' +ₗ 1) ⟧ᶜ }]]) -∗
    [[{ c ↦ #n ∗ mblist α Φx l ∗ q.[α] }]][inv_wsat ∗ borrow_wsat bupd ⟦⟧ᶜ]
      iter_mblist [f; #k; #c; #l]
